...
int main(void)
{
  ...
  // Kh?i t?o TM1637 v?i chân DIO là PA0, CLK là PA1
...
//  TM1637_DisplayDecimal(y);
  ...
  while (1)
	{
		HAL_Delay(500);
    int value = (int)(x * 100);  // Nhân 100 d? l?y 2 s? sau d?u ch?m

    TM1637_DisplayDigit(&tm1, (value / 1000) % 10, 0);  // Hàng nghìn
    TM1637_DisplayDigit(&tm1, (value / 100) % 10, 1);   // Hàng tram
    TM1637_DisplayDigit(&tm1, (value / 10) % 10, 2);    // Hàng ch?c
    TM1637_DisplayDigit(&tm1, value % 10, 3);           // Hàng don v?
		HAL_Delay(500);
